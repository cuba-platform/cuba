...
public class UserBrowser extends AbstractLookup {
    ...
    public void init(Map<String, Object> params) {
        ...
        if (params.containsKey("gen")) {
            usersTable.addGeneratedColumn("star", new Table.ColumnGenerator() {
                @Override
                public Component generateCell(Entity entity) {
                    if (((User) entity).getStar() == null) {
                        return null;
                    }

                    LinkButton button = componentsFactory.createComponent(LinkButton.NAME);
                    if (Boolean.TRUE.equals(((User) entity).getStar())) {
                        button.setIcon("icons/ok.png");
                    } else {
            ...
            usersTable.addGeneratedColumn("summary", new Table.ColumnGenerator() {
                @Override
                public Component generateCell(Entity entity) {
                    if (((User) entity).getStar() == null) {
                        return null;
                    ...
                    String summary = ((User) entity).getSummary();
                    LinkButton button = componentsFactory.createComponent(LinkButton.NAME);
            ...
            usersTable.addGeneratedColumn("userRoles", new Table.ColumnGenerator() {
                @Override
                public Component generateCell(Entity entity) {
                    LinkButton button = componentsFactory.createComponent(LinkButton.NAME);
                    button.setIcon("icons/copy.png");
                    return button;
                }
            });

            usersTable.addGeneratedColumn("state", new Table.ColumnGenerator() {
                @Override
                public Component generateCell(Entity entity) {
                    LinkButton button = componentsFactory.createComponent(LinkButton.NAME);
                    if (((User) entity).getState() != null) {
                        button.setCaption(((User) entity).getState());
                    }
                    return button;
                }
            });
            ...
            usersTable.addGeneratedColumn("summary", new Table.ColumnGenerator() {
                ...
                public Component generateCell(Entity entity) {
                    ...
                    return new Table.PlainTextCell(summary);
                }
            });
            usersTable.addGeneratedColumn("userRoles", new Table.ColumnGenerator() {
                @Override
            ...
            usersTable.addGeneratedColumn("state", new Table.ColumnGenerator() {
                @Override
                public Component generateCell(Entity entity) {
                    return new Table.PlainTextCell(((User) entity).getState());
                }
            });

            usersTable.setStyleProvider(new Table.StyleProvider() {
                @Nullable
                @Override
                    ...
                    if (entity != null && property != null) {
                        switch (property) {
                            case "star":
                                ...
                                if (Boolean.TRUE.equals(user.getStar())) {
                                    return "star-yes";
                                } else if (Boolean.FALSE.equals(user.getStar())) {
                                    return "star-no";
                                }
            ...
            usersTable.setClickListener("star", new Table.CellClickListener() {
                @Override
                public void onClick(Entity item, String columnId, int mouseX, int mouseY) {
                    showNotification("Star", NotificationType.HUMANIZED);
                }
            });
            usersTable.setClickListener("summary", new Table.CellClickListener() {
                @Override
                public void onClick(Entity item, String columnId, int mouseX, int mouseY) {
                    showNotification("Summary", NotificationType.HUMANIZED);
                }
            });
            usersTable.setClickListener("userRoles", new Table.CellClickListener() {
                @Override
                public void onClick(Entity item, String columnId, int mouseX, int mouseY) {
                    showNotification("User Roles", NotificationType.HUMANIZED);
                }
            });
            usersTable.setClickListener("state", new Table.CellClickListener() {
                @Override
                public void onClick(Entity item, String columnId, int mouseX, int mouseY) {
                    showNotification("State", NotificationType.HUMANIZED);
                }
            });
        }
    }
