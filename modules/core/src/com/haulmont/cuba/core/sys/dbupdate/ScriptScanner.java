...
public class ScriptScanner {
    ...
    public ScriptScanner(String dbScriptsDirectory, String storeName, String dbmsType, String dbmsVersion) {
    ...
    public List<ScriptResource> getScripts(ScriptType scriptType, @Nullable String moduleName) {
        try {
            ...
            String rootPath = String.format("%s/%s/%s/%s/",
                    ...
                    scriptTypeDirectory(scriptType),
                    dbmsType);
    ...
    protected String moduleDirectory(@Nullable String moduleName) {
        return moduleName != null ? moduleName : "**";
    }

    protected String scriptTypeDirectory(ScriptType scriptType) {
        String scriptTypeStr = scriptType.toString().toLowerCase(Locale.ROOT);
        if (Stores.MAIN.equals(storeName)) {
            return scriptTypeStr;
        } else {
            return String.format("%s_%s", scriptTypeStr, storeName.toLowerCase(Locale.ROOT));
        }
    }
