...
public class ReportEditor extends AbstractEditor {
  ...
  private def initValuesFormats() {
    final CollectionDatasource valuesFormatsDs = getDsContext().get('valuesFormatsDs')
    ...
    formatsHelper.createCreateAction([getValues: {
      [/*'position': valuesFormatsDs.itemIds.size(),*/
              'report': report]
    }, getParameters: {[:]}] as ValueProvider, WindowManager.OpenType.DIALOG)
