...
public class IEmbedded extends HTML implements Paintable {
    ...
    private final static String[] iframeBrowseMimeTypes = new String[] {
            "application/pdf"
    };
    ...
    public void updateFromUIDL(UIDL uidl, ApplicationConnection client) {
        ...
        if (uidl.hasAttribute("type")) {
            ...
            if (type.equals("image")) {
                ...
                clearBrowserElement = browseInIframe(uidl, client);
            } else {
            ...
            } else if (canBrowseInIframe(mime)) {
                clearBrowserElement =
                        browseInIframe(uidl, client);
            } else {
                ApplicationConnection.getConsole().log(
    ...
    private static boolean canBrowseInIframe(String mime) {
        for (final String mimeType : iframeBrowseMimeTypes) {
    ...
    private boolean browseInIframe(UIDL uidl, ApplicationConnection client) {
        boolean clearBrowserElement;
        if (browserElement == null) {
            setHTML("<iframe width=\"" + width + "\" height=\"" + height + "\" frameborder=\"0\" src=\""
                    + getSrc(uidl, client)
                    + "\" name=\""
                    + uidl.getId() + "\"></iframe>");
